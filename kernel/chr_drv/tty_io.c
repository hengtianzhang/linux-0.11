...
struct tty_struct tty_table[] = {
	{
		{ ICRNL, /* change incoming CR to NL 输入CR转NL*/
		  ...
		  INIT_C_CC }, //控制字符数组
		0,
		/* initial pgrp */ //所属初始进程组
		0,
		/* initial stopped */ //初始停止标志
		con_write, //控制台写函数
		{ 0, 0, 0, 0, "" },
		/* console read-queue */ //读缓冲
		{ 0, 0, 0, 0, "" },
		/* console write-queue */ //写缓冲
...
struct tty_queue *table_list[] = {
	&tty_table[0].read_q,  &tty_table[0].write_q, &tty_table[1].read_q,
	&tty_table[1].write_q, &tty_table[2].read_q,  &tty_table[2].write_q
};
