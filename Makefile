...
Image:bootsect setup system
	...
	dd if=system of=tools/a.img bs=512 count=4 seek=5 conv=notrunc
...
system:boot/head.o 
	gcc $(CFLAGS) -S -o init/main.s init/main.c
	as -o init/main.o init/main.s
	$(LD) $(LDFLAGS)  boot/head.o init/main.o -o system > system.map
head.o:boot/head.s
	as -o boot/head.o boot/head.s
clean:
	rm -f boot/bootsect  boot/setup  boot/*.o init/main.o system \
	system.map init/main.s
